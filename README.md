...
- PySpark processes and analyzes them in real-time by using Spark Streaming, and shows the results
...
I used simulated data for this project. The script iotsimulator.py generates JSON data in the following format:
...
temperature | Calculated by continuously adding a random number (between -1.0 to 1.0) to each state's average annual temperature every time when the data is generated.
...
## 3. Analysis of data
In this project, I implemented 4 types of real-time analysis:
...
- In the first .map operation, PySpark creates pair RDDs (k, v) where k is a value of a field state, and v is a value of a field temperature with a count of 1
...
- In the next .reduceByKey operation, PySpark aggregates the values by a same key and reduces them to a single entry  
...
- In the next .map operation, PySpark calculates the average temperature by dividing the sum of temperature by the total count  
...
- In the next .reduceByKey operation, PySpark aggregates the values by a same key and reduces them to a single entry but the values stay 1  
...
- In the last .reduceByKey operation, PySpark aggregates the values by a same key and reduces them to a single entry  
...
Akhila is a Data Analyst with over 3 years of experience specializing in analytics, business intelligence, and cloud-based data solutions. She has a proven track record of building scalable ETL pipelines and interactive dashboards across healthcare and insurance domains. Her expertise includes Python, SQL, Power BI, Snowflake, and the Azure data stack (Databricks, Data Factory, Synapse Analytics). This repository is maintained as part of her ongoing work in real-time data processing and stream analytics.